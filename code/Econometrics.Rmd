...
#export marg_pred
write.csv(marg_pred, "../output/marginal_predicted.csv")
...
show(marginal_fig)
ggsave("../output/marginal_effects.png")
```

```{r Predicted Values Plot}
predicted_fig <- ggplot(marg_pred, aes(x = bin, y = y_hat, group = label)) +
  geom_line(aes(color = label)) +
...
predicted_fig_ci <- ggplot(marg_pred, aes(x = bin, y = y_hat, group = label)) +
  geom_line(aes(color = label)) +
...
show(predicted_fig)
show(predicted_fig_ci)
ggsave("../output/predicted_values.png")
```
